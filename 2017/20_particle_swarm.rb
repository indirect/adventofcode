...
Particle = Struct.new(:id, :pos, :vel, :acc) do
  def self.from_line(id, line)
    p,v,a = line.split(", ").map do |var|
      var[3..-2].split(",").map(&:to_i)
    end
  ...
  def tick
    [0, 1, 2].each{|i| vel[i] += acc[i] }
    [0, 1, 2].each{|i| pos[i] += vel[i] }
  end
end

def closest(input)
  input.map.with_index do |line, id|
    [id, line.split(", ").last[3..-2].split(",").map(&:to_i)]
  end.min_by{|a| a[1].map(&:abs).sum }
end

puts "part 1"
raise "oh no" unless closest(testcase).first == 0
p closest(input).first
...
def undestroyed(input)
  ps = input.map.with_index{|line, id| Particle.from_line(id, line) }

  500.times do |i|
    print "." if (i % 10).zero?
    pos_list = Hash.new{|h, k| h[k] = [] }
    ps.each{|p| pos_list[p.pos].push(p) }

    ps = pos_list.values.select{|g| g.size < 2 }.flatten
    ps.each(&:tick)
  end
...
puts "part 2"
raise "oh no" unless undestroyed(testcase) == 1
p undestroyed(input)
