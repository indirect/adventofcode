...
Node = Struct.new(:name, :weight, :above)
Tree = Struct.new(:nodes, :below) do
  def self.parse(input)
    new({}, {}).tap do |t|
      input.each do |line|
        name, weight, above = line.scan(/(.+) \((\d+)\)(?: -> )?(.*+)/).flatten
        t.add_node(name, weight.to_i, above.split(", "))
      end
    end
  end

  def root
    root = below.values.first
    root = below[root] until below[root].nil?
    nodes[root]
  end

  def stack_weight_of(name)
    node = nodes[name]
    node.weight + node.above.map{|a| stack_weight_of(a) }.sum
  end

  def add_node(name, weight, above)
    above.each{|a| below[a] = name }
    nodes[name] = Node.new(name, weight, above)
  end
  ...
  def imbalanced_node
    ...
    loop do
      options = imbalanced.above.map{|n| [n, stack_weight_of(n)] }
      child = options.find{|n,w| options.map(&:last).count(w) == 1 }
      break if child.nil?
      imbalanced = nodes[child.first]
    end

    imbalanced = nodes[imbalanced.first]
    parent = below[imbalanced.name]
    options = nodes[parent].above.map{|n| [n, stack_weight_of(n)] }
    balanced_node = options.find{|n,w| options.map(&:last).count(w) > 1 }
    imbalanced_node = options.find{|n,w| options.map(&:last).count(w) == 1 }
    change = balanced_node.last - imbalanced_node.last
    result = nodes[imbalanced_node.first].weight + change

    [imbalanced_node.first, change, result]
  end
end
...
puts "part 2"
name, change, result = tree.imbalanced_node

puts "#{name} needs to change by #{change} and weigh #{result}"
