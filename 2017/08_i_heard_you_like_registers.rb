...
def execute(input, registers, highest_value = 0)
  input.each do |line|
    name, inst, amt, cond, cond_reg, cond_op, cond_amt = line.split(" ")
    amt = amt.to_i * (inst == "inc" ? 1 : -1)
    result = eval("registers['#{cond_reg}'] #{cond_op} #{cond_amt}")
    registers[name] += amt if result
    highest_value = registers[name] if registers[name] > highest_value
  end
...
puts "part 1"
highest = execute(input, registers)
